...
def categorize_details(details):
    ...
    if any(word in details for word in ["airtime", "tingg", "safaricom", "airtel", "bundles", "gessy"]):
        ...
        return "Rent Payment"
    elif any(word in details for word in ["cleanshelf","naivas", "tuskys", "quick mart", "carrefour", "4093275","supermarket", "shopping", "small business", "mall","jumia", "kilimall", "amazon", "shop", "market", "merchant", "direct pay"]):
        return "Shopping"
    elif any(word in details for word in ["baraka", "java", "hotel", "restaurant", "cafe"]):
        return "Restaurant"
    elif any(word in details for word in ["sacco", "uber"]):
        return "Savings"
    elif any(word in details for word in ["OLD MUTUAL LIFE ASSURANCE KENYA LIMITED"]):
        return "Savings"
    elif any(word in details for word in ["alpha", "water"]):
        return "Water"
    elif any(word in details for word in ["beauty", "nimo naturals", "bandari"]):
        return "Beauty"
